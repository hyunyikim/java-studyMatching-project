	...
	$(function() {
		...
		$('#studyUpdateBtn').click(function() {
			if ($('.studyAddText').val() == "" || $('#study_img').val() == "") {
				...
				$('#studyUpdateFrm').submit();
			}
		});
		
		$('#study_subject_code').val("${study.subject_name}").prop("selected", true); 
		$('#study_level_code').val("${study.level_name}").prop("selected", true); 
		$('#study_loc_code').val("${study.loc_name}").prop("selected", true); 
	});
</script>
<style>
#studyApplyWell {
...
<body>
<c:set var="study" value="${requestScope.study}"/>
	<div class="container" id="studyApplyContainer">
		...
		<br> <br>
		<form action="<%=request.getContextPath()%>/view/category/StudyUpdate.do"
			method="post" name="studyUpdateFrm" enctype="multipart/form-data"
			id="studyUpdateFrm">
			...
			<input type="file" class="form-control studyAddText"
				name="study_img" id="study_img"><br>
			<div style="text-align: right">
				입력하신 스터디 정보가 맞으시면 <b>스터디 수정하기 버튼</b>을 눌러주세요!&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" class="btn btn-green btn-flat" id="studyUpdateBtn"
					value="스터디 수정하기">
			</div>
		</form>
	</div>
